...
feature -- tests
	...
	t0: BOOLEAN
		...
		do
			...
			Io.put_string ("Create heap")
			check Result end

			Result := h.maximum = 16
			Io.put_string ("heap max = 16")
			check Result end

			Result :=
				h.max_capacity = 15 and h.count = 10 and not h.is_empty
			Io.put_string ("max capacity = 15 and heap count is 10 and heap not empty")
			Io.put_boolean (h.max_capacity = 15)
			Io.put_boolean (h.count = 10)
			Io.put_boolean (not h.is_empty)
			check Result end

			Result :=
				h.key_exists (10) and not h.key_exists (21)
				Io.put_string ("10 exists and 21 exists")
				Io.put_boolean (h.key_exists (10))
				Io.put_boolean (h.key_exists (21))
			check Result end
			...
			Io.put_string ("valid indexes?")
			check Result end

			Result :=
				not (h.is_valid_index (11) or h.is_valid_index (15) or h.is_valid_index (18))
			Io.put_string ("valid indexes 11 or 15 or 18")
			check Result end
			...
			Io.put_string ("has left child right child")
			check Result end
			...
			Io.put_string ("has parents")
			Io.new_line
			Io.put_boolean (h.has_parent (2))
			Io.put_boolean (h.parent_of (2) = 16)
			Io.put_boolean (h.has_parent (10))
			Io.put_boolean (h.parent_of (10) = 7)
			Io.put_boolean (not h.has_parent (1))
			check Result end
